...
def add_callback(phase):
    if phase == 'tf':
        weightsOutputFile = 'ModelCheckpoints/EfficientNET_tf.{epoch:02d}-{val_accuracy:.3f}.hdf5'
    elif phase == 'ft':
        weightsOutputFile = 'ModelCheckpoints/EfficientNET_ft.{epoch:02d}-{val_accuracy:.3f}.hdf5'
...
def setup_to_finetune_efficientNET(model):
...
def build_model(train_df, val_df):
    ...
    model = setup_to_finetune_efficientNET(model)
    # Train model
